...
//Step1: Append Data//
insheet using "$rawdata/ozone_chicago_20000101_20050101.txt", comma clear
save "$data/temp.dta", replace
insheet using "$rawdata/ozone_chicago_20050102_20091231.txt", comma clear
save "$data/temp2.dta", replace
insheet using "$rawdata/ozone_chicago_20100101_20121231.txt", comma clear
...
rename samplemeasurement ozone_hrly
label var ozone_hrly "Hourly concentration O3, ppm"

keep latitude longitude datum statecode countycode monitorid sitenum poc ozone_hrly date hour
compress

notes: O3 monitors in Chicago CBSA
notes: Keep only monitor-days with 18 or more hourly observations
notes: Generated by extract_chicago_ozone_2000_2012.do
save "$data/chicago_ozone_2000_2012_hourly.dta", replace
restore
...
bys monitorid datelocal sampleduration: egen num_hrly_obs_ozone = sum(observed)
bys monitorid datelocal sampleduration: egen max_ozone = max(samplemeasurement)
bys monitorid datelocal sampleduration: egen avg_ozone = mean(samplemeasurement)

label var max_ozone "Day's max hourly concentration O3, ppm"
label var avg_ozone "Day's avg hourly concentration O3, ppm"
...
notes: Generated by extract_chicago_ozone_2000_2012.do
compress
save "$data/chicago_ozone_2000_2012_daily.dta", replace
